...
class ModelPerformance(models.Model):
    ...
    format_compliance_count = models.PositiveIntegerField(default=0)  # total format checks
    format_compliance_passed = models.PositiveIntegerField(default=0)  # passed format checks
    ...
    def update_reliability_score(self):
        ...
        self.reliability_score = success_score + time_score + format_score + recent_score + failure_penalty
        self.reliability_score = max(0.0, min(1.0, self.reliability_score))  # Clamp between 0 and 1
...
class ResponseLog(models.Model):
    ...
    user_query_hash = models.CharField(max_length=64, null=True, blank=True)  # for consistency analysis
    created_at = models.DateTimeField(auto_now_add=True)
...
class ModelReliability(models.Model):
    ...
    circuit_breaker_threshold = models.PositiveIntegerField(default=5)  # consecutive failures before disabling
    ...
    def clean(self):
        ...
        if self.custom_temperature is not None and not (0.0 <= self.custom_temperature <= 2.0):
