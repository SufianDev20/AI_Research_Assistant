...
class PerformanceTracker:
    ...
    def log_request_start(
        model_name: str, request_type: str = "summary", user_query: str = None
            ...
            with transaction.atomic():
                ...
                if not created:
                    ...
                    if not perf.is_active:
                        perf.is_active = True
                        logger.info(
                            f"Circuit Breaker reset for %s after successful response",
                            model_name,
                        )
                # Update format compliance and reliability score (run once)
                ...
                logger.info(
                    f"Logged success for {model_name}: {response_time:.2f}s, reliability: {perf.reliability_score:.2f}"
                )
        ...
        try:
            with transaction.atomic():
                ...
                if not created:
                    ...
                    reliability = ModelReliability.objects.filter(
                        model_name=model_name
                    ).first()
                    if (
                        reliability
                        and perf.consecutive_failures
                        >= reliability.circuit_breaker_threshold
                    ):
                        perf.is_active = False
                        logger.warning(
                            f"Circuit breaker triggered for {model_name} after {perf.consecutive_failures} consecutive failures"
                        )
                ...
                logger.info(
                    f"Logged failure for {model_name}: {response_time:.2f}s, consecutive failures: {perf.consecutive_failures}"
                )
    ...
    def reset_circuit_breaker():
        """
        Re-enable models whose circuit breaker has been triggered after the ciruicuit breaker reset time has passed.
        A model that recovers from a transient outage should not stay permanently disabled if never called again.
        """
        cutoff = timezone.now() - timedelta(minutes=CIRCUIT_BREAKER_RESET_MIN)
        recovered = ModelPerformance.objects.filter(
            is_active=False,
            last_failure__lte=cutoff,
    ...
    def get_intelligent_model_order(free_models: List[str]) -> List[str]:
        ...
        try:
            ...
            performance_models = ModelPerformance.objects.filter(
                model_name__in=free_models, is_active=True
            ...
            for perf_model in performance_models:
                if perf_model.model_name in model_scores:
                    ...
                    model_scores[perf_model.model_name] = (
                        perf_model.reliability_score * 300
                    )
            ...
            logger.info(
                f"Intelligent model order: {ordered_models[:5]}..."
            )  # Log first 5
    ...
    def get_model_temperature(
        model_name: str, default_temperature: float = 0.3
    ) -> float:
        """
    ...
    def get_model_stats() -> Dict:
        ...
        try:
            stats = {
                ...
                "active_models": ModelPerformance.objects.filter(
                    is_active=True
                ).count(),
                "models_by_tier": {},
                "top_performers": [],
            ...
            top_models = ModelPerformance.objects.filter(
                is_active=True, total_requests__gte=5  # At least 5 requests
