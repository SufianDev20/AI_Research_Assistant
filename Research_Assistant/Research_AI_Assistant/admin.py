...
from .models import QueryLog, ModelPerformance, ResponseLog, ModelReliability, PaperPDF
...
class QueryLogAdmin(admin.ModelAdmin):
    ...
    def query_text_short(self, obj):
        return (
            obj.query_text[:50] + "..." if len(obj.query_text) > 50 else obj.query_text
        )
...
class ModelPerformanceAdmin(admin.ModelAdmin):
    ...
    def reliability_score(self, obj):
        try:
            ...
            return mark_safe(
                f'<span style="color: {color}; font-weight: bold;">{score:.3f}</span>'
            )
        except (ValueError, TypeError):
            return mark_safe('<span style="color: red;">N/A</span>')
...
class ResponseLogAdmin(admin.ModelAdmin):
    ...
    def response_time(self, obj):
        try:
            ...
            return mark_safe(
                f'<span style="color: {color}; font-weight: bold;">{time_ms:.0f}ms</span>'
            )
        except (ValueError, TypeError):
            return mark_safe('<span style="color: red;">N/A</span>')
...
class ModelReliabilityAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Configuration", {"fields": ("model_name", "tier", "priority")}),
        (
            "Advanced Settings",
            {
                "fields": (
        ...
        ("System Information", {"fields": ("last_updated",), "classes": ("collapse",)}),
    )
...
@admin.register(PaperPDF)
class PaperPdfAdmin(admin.ModelAdmin):
    list_display = ["openalex_id", "page_count", "created_at"]
    list_filter = ["extraction_success"]
    search_fields = ["openalex_id", "pdf_url"]
    readonly_fields = ["markdown_content", "image_paths", "created_at"]
