...
def test_openrouter_performance():
    ...
    try:
        ...
        if not missing_fields:
            ...
            print(
                f"- Missing {len(missing_fields)} required fields: {', '.join(missing_fields)}"
            )
        return True
...
def check_performance_data():
    ...
    try:
        from Research_AI_Assistant.services.performance_tracker import (
            PerformanceTracker,
        )
        ...
        for model in stats.get("top_performers", [])[:3]:
            if model.get("total_requests", 0) > 0:
                print(
                    f"  {model['model']}: {model['reliability_score']:.3f} ({model['success_rate']:.1f}% success)"
                )
        ...
        for model in models_with_data:
            ...
            if model.format_compliance_count > 0:
                print(
                    f"    Format compliance: {model.format_compliance_score:.1%} ({model.format_compliance_passed}/{model.format_compliance_count} checks passed)"
                )
            else:
        ...
        for log in recent_logs:
            ...
            print(
                f"  {status} {log.model_name} - {log.request_type} ({log.response_time:.2f}s)"
            )
...
def main():
    ...
    if success:
        ...
        print(
            "2. View performance stats at http://127.0.0.1:8080/api/performance/stats/"
        )
